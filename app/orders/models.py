...
class Chat(Base, Dated):
    __tablename__ = "chats"
    id: Mapped[UUID] = mapped_column(UUID, primary_key=True, index=True, default=uuid4)
    tg_chat_id: Mapped[int]
    messages: Mapped[list["Message"]]
    orders: Mapped[list["Order"]] = relationship(back_populates="chats")
...
class Order(Base, Dated):
    __tablename__ = "orders"
    id: Mapped[UUID] = mapped_column(UUID, primary_key=True, index=True, default=uuid4)
    tg_chat_id: Mapped[int]
    messages: Mapped[list["Message"]]
    chat_id: Mapped[UUID] = mapped_column(ForeignKey("chats.id", ondelete="CASCADE"), index=True)
    chat: Mapped["Chat"] = relationship(back_populates="orders")
    ...
    positions: Mapped[Positions] = relationship(back_populates="order")
